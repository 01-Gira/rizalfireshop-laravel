...
class CheckoutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($cart == null){
            ...
            return redirect()->back()->with('error','You need to log in first to procced the orders');
        }else{
            $cart = session('cart.items', []);
            $subTotal = 0;
            $weight = 0;
    ...
    public function store(Request $request)
    {
        ...
        return redirect('/orders')->with('success', 'Order with order id ' . $order->order_id . ' has been created! Please procced with the payment');
    }
